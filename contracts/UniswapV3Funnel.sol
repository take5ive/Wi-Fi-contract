...
import "./uniswapV3/v3-periphery/contracts/interfaces/INonfungiblePositionManager.sol";
import "./uniswapV3/v3-periphery/contracts/libraries/TransferHelper.sol";
import "./uniswapV3/v3-core/contracts/UniswapV3Pool.sol";
import "./interfaces/IWETH9.sol";
import "./uniswapV2/periphery/libraries/UniswapV2LibraryV8.sol";
import "../contracts/priceOracle/PriceOracleTick.sol";
import "./uniswapV3/v3-periphery/contracts/libraries/SqrtPriceMathPartial.sol";
...
contract UniswapV2Funnel {
    ...
    function _transferFromSenderOrWrap(
        ...
        if (token == WETH) {
            ...
            TransferHelper.safeTransfer(WETH, to, amount); //---TODO: check
        } else {
    ...
    function partitionAndAddLiquidity(
        ...
        address factory = IUniswapV3Pool(pair).factory();
        address farmToken = IUniswapV3Pool(pair).token0() == baseToken
            ? IUniswapV3Pool(pair).token1()
            : IUniswapV3Pool(pair).token0();
        baseToken = IUniswapV3Pool(pair).token0() == baseToken
            ? IUniswapV3Pool(pair).token0()
            : IUniswapV3Pool(pair).token1();
        if (baseToken == WETH) {
        ...
        rBase = IUniswapV3Pool(pair).balance0();
        rFarm = IUniswapV3Pool(pair).balance1();
        ...
        // 3-3. mint liquidity
        liquidity = IUniswapV3Pool(pair).mint(to);
    }
     ...
     * @param reserve0 token0 Reserved Amount in Pair
     * @param reserve1 token1 Reserved Amount in Pair
    ...
    function optimalSwapAmount(
        ...
        if (amount0 * reserve1 >= amount1 * reserve0) {
            ...
            swappedAmount = SwapMath.computeSwapStep(
                sqrtRatioCurrentX96,
                sqrtRatioTargetX96,
                liquidity,
                swapAmount,
    ...
    function decomposeAndAddLiquidity(
        ...
        {
            (uint112 reserveb0b, uint112 reserveb00, ) = IUniswapV3Pool(pairb0)
                .getReserves();
            if (baseToken > farm0)
        ...
        {
            (uint112 reserveb1b, uint112 reserveb11, ) = IUniswapV3Pool(pairb1)
                .getReserves();
            if (baseToken > farm1)
        ...
        {
            (uint112 reserve010, uint112 reserve011, ) = IUniswapV3Pool(pair01)
                .getReserves();
        ...
        liquidity = IUniswapV3Pool(pair01).mint(to);
    }
    ...
    function removeLiquidityAndSwapToDstToken(
        ...
        address factory = IUniswapV3Pool(lpAddress).factory();
        uint32 feeBps = feeOf[factory];
        //* 1. remove liquidity
        //* path1[0] : baseToken , path2[0]: farmToken
        IUniswapV3Pool(pair).transferFrom(msg.sender, pair, liquidity);
        (uint amount0, uint amount1) = IUniswapV3Pool(pair).burn(address(this));
    ...
    function removeLiquidityAndSwapToETH(
        ...
        address factory = IUniswapV3Pool(lpAdress).factory();
    ...
    function _swapFromSender(
        ...
        if (tokenIn < tokenOut) {
            // zeroForOne: true, sqrtPriceLimitX96: 0 , token1/token0 > 0
            IUniswapV3Pool(pair).swap(to, true, amountIn, 0, new bytes(0));
        } else {
            // zeroForOne: false , sqrtPriceLimitX96: max , token1/token0 < max
    ...
    function _swap(
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ...
                : _to;
            IUniswapV3Pool(UniswapV2Library.pairFor(factory, input, output))
    ...
    function _swapExactTokensForTokens(
        ...
        TransferHelper.safeTransfer(
            ...
            PoolAddress.computeAddress(
                factory,
                PoolKey{token0: path[0], token1: path[1], fee: feeBps}
            ),
            amounts[0]
        );
        for (uint i; i < path.length - 1; i++) {
        ...
        )
    {
        ...
        {
            (uint112 reserveb0b, uint112 reserveb00, ) = IUniswapV3Pool(
        ...
        {
            (uint112 reserveb1b, uint112 reserveb11, ) = IUniswapV3Pool(
        ...
        {
            (uint112 reserve010, uint112 reserve011, ) = IUniswapV3Pool(
        ...
        {
            uint _totalSupply = IUniswapV3Pool(
                UniswapV2Library.pairFor(factory, farm0, farm1)
            ).totalSupply();
    ...
    function calculateDstAmountByRemoveLiquidity(
        ...
        uint32 feeBps = feeOf[factory];
        (uint112 balance0, uint112 balance1, ) = IUniswapV3Pool(pair)
            .getReserves();

        uint _totalSupply = IUniswapV3Pool(pair).totalSupply();
